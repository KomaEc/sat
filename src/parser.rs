...
pub fn dimacs_parser<R>(br: BufReader<R>) -> io::Result<(usize, usize, Vec<Vec<i32>>)>
where
    R: Read,
{
    // parse comment
    ...
    let mut lines = br.lines().skip_while(|line| {
    ...
    let mut line: String = lines
        .next()
        .expect("expect problem definition")
        .expect("expect problem definition");
    ...
    n_vars = problem[2]
        .parse::<usize>()
        .expect("expect number of variables");
    n_clauses = problem[3]
        .parse::<usize>()
        .expect("expect number of clauses");
    ...
    for _ in 0..n_clauses {
        ...
        loop {
            line = lines
                .next()
                .expect(&format!("expect {} clauses", n_clauses))
                .expect(&format!("expect {} clauses", n_clauses));
            ...
            dimacs_clause.extend_from_slice(
                &previous
                    .iter()
                    .map(|s| s.parse::<i32>().expect("literal"))
                    .collect::<Vec<_>>()[..],
            );
...
#[allow(dead_code)]
pub fn dimacs_printer(n_vars: usize, n_clauses: usize, clauses: &Vec<Vec<i32>>) -> String {
...
mod tests {
    ...
    fn sat_instance() -> impl Strategy<Value = (usize, usize, Vec<Vec<i32>>)> {
        (3usize..16, 5usize..40).prop_flat_map(|(n_vars, n_clauses)| {
